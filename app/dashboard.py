...
def render_digital_mrv():
    st.markdown('<div class="section-kicker">DIGITAL MRV / EQUIPMENT USERS</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-title">For farmers who have purchased our equipment</div>', unsafe_allow_html=True)
    st.markdown('<p class="small-copy">This workflow uses equipment-backed field context plus Sentinel-1 SAR evidence. Trigger the calculation only when you want a result; the technical working stays hidden until you open it.</p>', unsafe_allow_html=True)

    chosen = st.radio("Workflow", ["I purchased Code4Nature equipment", "I have not purchased equipment"], index=0, horizontal=True, key="mrv_equipment_status")
    if chosen.startswith("I have not"):
        st.info("The Farm Simulator is the right section for you. It estimates the opportunity before equipment purchase.")
        if st.button("Open Farm Simulator", use_container_width=True):
            st.session_state["site_page"] = "Farm simulator"
            st.rerun()
        return

    vv = st.number_input("VV mean (dB)", value=-17.60, step=0.01, format="%.2f")
    vh = st.number_input("VH mean (dB)", value=-23.80, step=0.01, format="%.2f")

    if st.button("Run MRV calculation", type="primary", use_container_width=True):
        st.session_state["mrv_calc"] = {
            "vv": vv,
            "vh": vh,
            "wetness": max(0.0, min(100.0, 50.0 + (vv + 15.0) * 7.0)),
        }

    result = st.session_state.get("mrv_calc")
    if result:
        x1, x2, x3 = st.columns(3)
        x1.metric("Relative wetness", f"{result['wetness']:.1f}%")
        x2.metric("VV mean", f"{result['vv']:.2f} dB")
        x3.metric("VH mean", f"{result['vh']:.2f} dB")

        with st.expander("Show technical calculation / BTS"):
            st.code(
                "relative_wetness = clamp(50 + (VV + 15) × 7, 0, 100)\n"
                f"VV = {result['vv']:.2f} dB\n"
                f"relative_wetness = {result['wetness']:.1f}%\n"
                f"VH = {result['vh']:.2f} dB"
            )
            st.markdown(
                "**VH mean:** VH is the cross-polarized SAR backscatter measurement; it complements VV when interpreting vegetation and surface scattering."
            )
            st.caption("This is a relative wetness proxy for the demonstrator, not an absolute soil-moisture measurement without field calibration.")

    st.markdown('<div class="section-kicker" style="margin-top:2rem">CONSULTING</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="info-card"><b>Need equipment onboarding or project MRV support?</b><div class="small-copy" style="margin-top:.5rem;">Book a consultation for farm onboarding, data setup, MRV workflow design and project support.</div><div style="margin-top:1rem;">Email: <a href="mailto:hello@code4nature.earth">hello@code4nature.earth</a><br>Phone: add your consulting number</div></div>',
        unsafe_allow_html=True,
    )


def render_farm_simulator():
    st.markdown('<div class="section-kicker">FARM SIMULATOR / NON-EQUIPMENT USERS</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Estimate your carbon opportunity before buying equipment.</div>', unsafe_allow_html=True)
    st.markdown('<p class="small-copy">Use this section to understand the opportunity from your farm data. Once you purchase our equipment, move to Digital MRV for equipment-backed measurements.</p>', unsafe_allow_html=True)

    chosen = st.radio("Workflow", ["I have not purchased equipment", "I purchased Code4Nature equipment"], index=0, horizontal=True, key="sim_equipment_status")
    if chosen.startswith("I purchased"):
        st.success("Digital MRV is the right section for you.")
        if st.button("Open Digital MRV", use_container_width=True):
            st.session_state["site_page"] = "Digital MRV"
            st.rerun()
        return

    area = st.number_input("Farm area (hectares)", min_value=0.01, value=float(st.session_state.get("farm_area_ha", 10.0)), step=0.1)
    baseline = st.number_input("Baseline emissions (tCO2e/ha/year)", min_value=0.1, value=6.0, step=0.1)
    reduction_pct = st.slider("Modelled emission reduction (%)", 0, 70, 41, 1)
    ...
    live_price = float(market.get("market_carbon_price_usd") or 0.0)
    source = market.get("market_source", "market feed")
    st.markdown(
        f'<div class="info-card"><div class="section-kicker">LIVE CARBON RATE</div><div style="font-size:2rem;font-weight:900;">USD {live_price:,.2f} <span style="font-size:.9rem;color:#8ca99a;">/ tCO2e</span></div><div class="small-copy" style="font-size:.78rem;margin-top:.3rem;">{source} · used dynamically for this scenario</div></div>',
        unsafe_allow_html=True,
    )

    if st.button("Run farm opportunity calculation", type="primary", use_container_width=True):
        annual_credits = area * baseline * (reduction_pct / 100.0)
        st.session_state["farm_opportunity"] = {
            "annual_credits": annual_credits,
            "annual_value": annual_credits * live_price,
            "price": live_price,
            "area": area,
            "reduction_pct": reduction_pct,
        }

    opportunity = st.session_state.get("farm_opportunity")
    if opportunity:
        years_past = st.slider("Years farming without this method", 1, 25, 5, 1)
        years_future = st.slider("Future years using our method", 1, 25, 5, 1)
        past_value = opportunity["annual_value"] * years_past
        future_value = opportunity["annual_value"] * years_future

        p1, p2 = st.columns(2)
        p1.metric("Estimated carbon value potentially forgone", f"USD {past_value:,.0f}")
        p2.metric("Projected carbon value with the method", f"USD {future_value:,.0f}")

        st.caption(
            f"Based on {opportunity['annual_credits']:.2f} modelled credits/year at the live VCM reference rate of USD {opportunity['price']:.2f}/tCO2e. This is a scenario projection, not a reconstruction of historical prices or a guarantee of future prices."
        )

        with st.expander("Show technical calculation / BTS"):
            st.code(
                f"annual_credits = area × baseline_emissions × reduction%\n"
                f"= {opportunity['area']:.2f} × {baseline:.2f} × {opportunity['reduction_pct']}%\n"
                f"= {opportunity['annual_credits']:.2f} tCO2e/year\n"
                f"annual_value = annual_credits × live_price\n"
                f"= {opportunity['annual_value']:.2f} USD/year\n"
                f"past_value = annual_value × past_years\n"
                f"future_value = annual_value × future_years"
            )

        st.success("Thank you for making an effort to save Mother Earth. Your field data helps us build a stronger evidence base for climate-smart rice.")

    st.markdown('<div class="section-kicker" style="margin-top:2rem">CONSULT WITH CODE4NATURE</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="info-card"><b>Ready to move from simulation to equipment-backed MRV?</b><div class="small-copy" style="margin-top:.5rem;">Book a consultation for equipment selection, farm onboarding, data setup and carbon-MRV planning.</div><div style="margin-top:1rem;">Email: <a href="mailto:hello@code4nature.earth">hello@code4nature.earth</a><br>Phone: add your consulting number</div></div>',
        unsafe_allow_html=True,
    )
